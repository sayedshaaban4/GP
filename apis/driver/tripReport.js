...
tripReportRouter.post('/api/driver/trip-report' ,token , async (req , res) => {
    try{
        ...
        const {drowsinessRate  , distractionRate , calling , texting , drinking , reachingBehind , source , destination , date , time , duration} = req.body;
        
        const userName = user.userName
        const total = (((200.0 - (distractionRate + drowsinessRate)) / 200.0 ) * 100.0)
